...
def subdomainsearch_360e(domain):
    def danyedayin(zhuyuming,changdu):#搜索出的单页结果打印
        url='http://en.so.com/s?q=site:'+zhuyuming+'&pn='+changdu
        try:
    ...
    def wnjianqucong():#文件去重
        outfile = open('360e_jiguoIP.txt', 'w') #新的文件
        list_1=[]
        for line in open('360e_foo.txt'):  #老文件
            tmp = line.strip()
    ...
    def jiacom():
        f = open("360e_jiguoIP.txt", "r")
        fopen = open("360e_jieguo.txt", "w")
        while True:
            line = f.readline()
            if line:
                line=line.strip()
    ...
    wnjianqucong()
    wnjianhangshu()
    os.remove('360e_foo.txt')
    jiacom()
    os.remove('360e_jiguoIP.txt')

if __name__ == "__main__":
    subdomainsearch_360e()